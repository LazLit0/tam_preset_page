TEST_CASE ("Boot performance")
{
    ...
    {
        std::vector<Catch::Benchmark::storage_for<JucePresetManagerAudioProcessor>> storage (size_t (meter.runs()));
    ...
    (Catch::Benchmark::Chronometer meter)
    {
        std::vector<Catch::Benchmark::destructable_object<JucePresetManagerAudioProcessor>> storage (size_t (meter.runs()));
        for (auto& s : storage)
            s.construct();
    ...
    (Catch::Benchmark::Chronometer meter)
    {
        JucePresetManagerAudioProcessor plugin;
